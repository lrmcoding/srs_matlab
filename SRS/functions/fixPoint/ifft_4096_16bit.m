function [data_out,nShift] = ifft_4096_16bit(datain,size,path1)
% size:ifft点数
% path1:文件路径
    ...
    for iprog = 1:prog
        ...
        if max(real_data)>=16384||min(real_data)<=-16384||max(imag_data)>=16384||min(imag_data)<=-16384
            data_out = limit(data_out,16,-1);
            nShift = nShift+1;
            fprintf(fp,'%d',nShift);
%             disp(nShift)
        else
